...
class TestSweep(unittest.TestCase):
    ...
    @unittest.skipIf("DATA_DIR" not in os.environ, "needs DATA_DIR environment " "variable")
    def test_end_to_end(self):
        output_dir = os.path.join("/tmp", str(uuid.uuid4()))
        result = subprocess.run(
            f"python -m domainbed.scripts.sweep launch "
        ...
        # get relaunched.
        subprocess.run(
            f"python -m domainbed.scripts.sweep delete_incomplete "
            f'--data_dir={os.environ["DATA_DIR"]} --output_dir={output_dir} '
            f"--algorithms ERM --datasets Debug28 --n_hparams 1 --n_trials 1 "
            f"--command_launcher dummy --skip_confirmation",
            shell=True,
