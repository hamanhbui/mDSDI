...
from domainbed.lib.misc import ParamDict, random_pairs_of_minibatches
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches):
            ...
            inner_opt = torch.optim.Adam(
                inner_net.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams["weight_decay"]
            )
...
class IGA(ERM):
    ...
    def update(self, minibatches, unlabeled=False):
        ...
        mean_loss = total_loss / len(minibatches)
        mean_grad = autograd.grad(mean_loss, self.network.parameters(), create_graph=True)
