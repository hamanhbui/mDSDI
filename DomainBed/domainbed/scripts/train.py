...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="domain_generalization", choices=["domain_generalization", "domain_adaptation"]
    )
    parser.add_argument("--hparams", type=str, help="JSON-serialized hparams dict")
    parser.add_argument(
        "--hparams_seed", type=int, default=0, help='Seed for random hparams (0 means "default hparams")'
    )
    parser.add_argument(
        "--trial_seed",
        type=int,
    ...
    parser.add_argument(
        "--checkpoint_freq", type=int, default=None, help="Checkpoint every N steps. Default is dataset-dependent."
    )
    ...
    train_loaders = [
        InfiniteDataLoader(
            dataset=env, weights=env_weights, batch_size=hparams["batch_size"], num_workers=dataset.N_WORKERS
        )
    ...
    for step in range(start_step, n_steps):
        ...
        if hparams["mDSDI"]:
            minibatches_device = [
                (x.to(device), y.to(device), z.to(device)) for x, y, z in next(train_minibatches_iterator)
            ]
        else:
