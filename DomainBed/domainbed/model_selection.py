...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def _step_acc(self, record):
        ...
        test_in_acc_key = "env{}_in_acc".format(test_env)
        return {"val_acc": np.mean([record[key] for key in val_env_keys]), "test_acc": record[test_in_acc_key]}
...
class LeaveOneOutSelectionMethod(SelectionMethod):
    ...
    def run_acc(self, records):
        step_accs = (
            records.group("step").map(lambda step, step_records: self._step_acc(step_records)).filter_not_none()
        )
        if len(step_accs):
