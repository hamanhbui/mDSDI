...
class TestQuery(unittest.TestCase):
    def test_everything(self):
        numbers = Q([1, 4, 2])
        people = Q([{"name": "Bob", "age": 40}, {"name": "Alice", "age": 20}, {"name": "Bob", "age": 10}])
        ...
        self.assertEqual(set(people.group("name").map(lambda _, g: g.select("age").mean())), set([25, 20]))

        self.assertEqual(people.argmax("age"), people[0])

    def test_group_by_unhashable(self):
        jobs = Q([{"hparams": {1: 2}, "score": 3}, {"hparams": {1: 2}, "score": 4}, {"hparams": {2: 4}, "score": 5}])
        grouped = jobs.group("hparams")
        self.assertEqual(grouped, [({1: 2}, [jobs[0], jobs[1]]), ({2: 4}, [jobs[2]])])
